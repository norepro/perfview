...
{
    /// <summary>
    /// Interaction logic for CallerCalleeView.axaml
    /// </summary>
    public partial class CallerCalleeView : UserControl
    {
        ...
        #region private
        private void CallerCallee_MouseDoubleClick(object sender, TappedEventArgs e) // TODO_AVALONIA: verify event args (was MouseButtonEventArgs)
        {
            var control = sender as Control; // TODO_AVALONIA: DependencyObject → Control
            var stackWindow = control.FindAncestorOfType<StackWindow>(); // TODO_AVALONIA: AncestorOfType → FindAncestorOfType
            if (stackWindow == null)
            {
                return;
            }
